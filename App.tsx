...
import { ShoppingBag, ArrowRight, Star, ArrowLeft, CreditCard, CheckCircle2, Search, Filter, Package, Cpu } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const [user, setUser] = useState<User | null>({
    ...
    address: '21 Nebula Heights, Metaverse City',
    isAdmin: true
  });
      ...
      {cart.length > 0 ? (
        <div className="space-y-6">
          ...
          <div className="glass rounded-3xl p-8 border border-white/10 space-y-6">
            ...
            <button onClick={() => setView('checkout')} className="w-full py-4 bg-cyan-600 hover:bg-cyan-500 rounded-2xl font-bold transition-all shadow-lg flex items-center justify-center gap-2">Initiate Checkout Protocol <ArrowRight className="w-5 h-5" /></button>
          </div>
        </div>
      ) : (
  ...
  const renderProfile = () => (
    <div className="max-w-4xl mx-auto px-6 py-32 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="glass rounded-3xl p-8 border border-white/10 mb-8 flex flex-col md:flex-row items-center gap-8">
        <div className="w-32 h-32 rounded-full overflow-hidden border-4 border-cyan-500/20">
          <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${user?.name}`} className="w-full h-full object-cover" />
        </div>
        <div><h1 className="text-3xl font-black mb-2">{user?.name}</h1><p className="text-slate-400 mb-4">{user?.email}</p></div>
      </div>
      <div className="space-y-6">
        <h2 className="text-2xl font-black">History Logs</h2>
        {orders.length > 0 ? (
          <div className="space-y-4">
            {orders.map(o => (
              <div key={o.id} className="glass rounded-2xl p-6 border border-white/10">
                <div className="flex justify-between mb-4"><span className="font-mono text-cyan-400">#{o.id.toUpperCase()}</span><span className="font-bold">${o.total}</span></div>
                <div className="text-xs uppercase tracking-widest text-slate-500">{o.status} - {o.date}</div>
              </div>
            ))}
          </div>
        ) : <p className="text-slate-500 text-center">No logs found.</p>}
      </div>
    </div>
  );
